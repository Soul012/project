...
class UserInfo {
        ...
        try {
            ...
            const options = {
                ...
                headers: {
                    ...
                    'User-Agent':
                        'Mozilla/5.0 (iPhone; CPU iPhone OS 17_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.47(0x18002f28) NetType/WIFI Language/zh_CN',
                    memberSession: this.token
                },
                body: `{}`
            }
            console.log(options)
        ...
        try {
            const options = {
                ...
                headers: {
                    'Content-Type': 'application/json',
                    'User-Agent':
                        'Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.42(0x18002a2a) NetType/WIFI Language/zh_CN',
                    memberSession: this.token
                },
                body: '{"memberInfoSource":2}'
            }
            const { data, code, msg } = (await httpRequest(options)) ?? {}
            if (code === 200) {
                this.userId = data?.nickName
                console.log(`用户名: ${data.nickName}`)
            } else {
    ...
    try {
        const options = {
            url: `https://member-pro.zhengjiamax.com/memberSign/getSignList`,
            headers: {
                'Content-Type': 'application/json',
                'User-Agent':
                    'Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.42(0x18002a2a) NetType/WIFI Language/zh_CN',
        ...
        const { data, code } = (await httpRequest(options)) ?? {}
        if (code === 200) {
            this.sign = data?.continueSign
            console.log(`连续签到天数: ${data.continueSign}`)
        } else {
...
async function getCookie() {
    $.isReuqest = () => typeof $request !== 'undefined'
    ;(async () => {
        if ($.isReuqest()) {
            const { membersession: tokenValue } = ObjectKeys2LowerCase($request.headers)
            if (tokenValue) {
...
async function getNotice() {
    try {
        const urls = [
            'https://raw.githubusercontent.com/shufflewzc/faker/main/grand.json',
            'https://raw.githubusercontent.com/anyehttp/quantumult-x/main/miscellaneous/exegesis/tips-disclaimer.json'
        ]
