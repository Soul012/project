    ...
    for (let user of $.userList) {
        ...
        try {
            ...
            if (user.ckStatus) {
                ...
                DoubleLog(`[${username}] 金豆: ${point}  等级: Lv${level || 0}\n距离下一级「Lv${level + 1}」还差${nextLevel}金豆`);
...
class UserInfo {
    ...
    async getUserInfo() {
        try {
            const opts = {
                url: "/member-hp-webapi/v1/user/getUserProfileInformation",
                params: { lenovoId: this.userId },
                type: "post",
            }
    ...
    async getPoint() {
        try {
            const opts = {
                url: "/member-hp-webapi/v1/userBenefit/getMyAssets",
                type: "post",
            }
            let res = await this.fetch(opts);
            return res?.data?.ledouNum;
        } catch (e) {
            this.ckStatus = false;
            $.log(`[${this.userName || this.index}][ERROR] ${e}\n`);
        }
    ...
    async signin() {
        try {
            const opts = {
                url: `https://mmembership.lenovo.com.cn/member-hp-task-center/v1/task/checkIn?lenovoId=${this.userId}&OSType=10011`,
                type: "post",
            }
            let res = await this.fetch(opts);
    ...
    async getTaskList() {
        try {
            const opts = {
                url: `/member-hp-task-center/v1/task/getUserTaskList`,
                type: "post",
            }
            let res = await this.fetch(opts);
    ...
    async doTask(taskId) {
        try {
            const opts = {
                url: `/member-hp-task-center/v1/checkin/selectTaskPrize`,
